...
export default function ControlPanel({
  ...
  useEffect(() => {
    if (analysisData && analysisData.checks && showResults) {
      ...
      const mappedChecks = analysisData.checks.map((check: any) => ({
        ...
        description:
          check.details ||
          checks.find((c) => c.id === check.id)?.description,
      }));
      ...
      if (analysisData.autoStartAI && analysisData.aiAnalysisPromise) {
        ...
        analysisData.aiAnalysisPromise
          .then(async (aiResponse: any) => {
            if (aiResponse) {
              ...
              if (data.success && data.insights) {
                const aiChecks: CheckItem[] = data.insights.map(
                  (insight: any, idx: number) => ({
                    ...
                    description:
                      insight.details?.substring(0, 60) + "..." ||
                      "AI Analysis",
                    isAI: true,
                  }),
                );
                ...
                if (data.insights.length > 0) {
                  ...
                  const avgAiScore =
                    aiScores.reduce((a: number, b: number) => a + b, 0) /
                    aiScores.length;
                  const combinedScore = Math.round(
                    overallScore * 0.6 + avgAiScore * 0.4,
                  );
          ...
          .catch((error) => {
            ...
            setCombinedChecks((prev) =>
              prev.filter((c) => !(c as any).isLoading),
            );
          })
          .finally(() => {
  ...
  const getStatusIcon = (status: CheckItem["status"]) => {
    switch (status) {
      ...
      default:
        return (
          <div className="w-16 h-16 rounded-full border border-black-alpha-8" />
        );
    }
  };
              ...
              >
                <div className="relative">
                  ...
                  <h3 className="text-label-large mb-4 text-accent-black font-medium flex items-center gap-6">
                    {check.label}
                    {check.tooltip &&
                      !aiInsights.some((ai) => ai.id === check.id) && (
          ...
          >
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="mt-16 text-center">
                <div className="text-title-h3 text-accent-black">
                  {overallScore}%
                </div>
            ...
            {aiInsights.length > 0 && (
              ...
              >
                <div className="text-label-large text-black-alpha-32 font-medium">
                  VS
                </div>
              </motion.div>
            )}
